function [ histPos, histPot ] = simulacion(gridsize,initsize,step,N,T,dt,w,alpha,beta,gamma,K,Utot,artificialMin)
%% Simulacion de PSO
%pause(5);
...
while (t>T-5 && t<T-2)
    for i = 1:N
        ...
        V(:,i) = K*(w*V(:,i)+alpha*rand(2,1).*(x_loc-X(:,i))+...
            rand(2,1).*beta.*([-1;9]-X(:,i))+gamma*[(rand(1,1)-0.5)*1;(rand(1,1)-0.5)*1]); % actualización de velocidad (aquí se coloca el modelo)
        %V(:,i) = w*V(:,i)+alpha*rand(2,1).*(x_loc-X(:,i));
        possible_nextloc =  X(:,i) + V(:,i)*dt;
        possibleN = mapXtoN(possible_nextloc,gridsize,step);
        possible_gradient = Utot(possibleN(1),possibleN(2));
