function [Utot] = casoA(gridsize,step,rounding,zeta,eta,dstar,Qi,c_g,l_g,c_o,l_o,choset,behaviour,showAPF)
%% Caso a resolver A
...
%plot(Xrep(on_total),Yrep(on_total),'bo')
...
diY = zeros(size(Y1));
for i = 1:size(X1,1)
    diX(:,i) = repmat(min(abs(repmat(X1(1,i), size(edges,1),1)-edges(:,1))),size(X1,1),1);
    diY(i,:) = repmat(min(abs(repmat(Y1(i,1), size(edges,1),1)-edges(:,2))),1,size(Y1,2));
end

%Se calcula la distancia euclediana de todos los puntos al obstaculo
...
if (behaviour == 1)
    ...
    if (choset == 1)
        Utot = (Urep+Urepw).*Uatt1/100+Uatt1;
    end
else
    Utot = (Urep+Urepw)+Uatt1;
end

title('Caso A', 'Interpreter', 'latex', 'Fontsize', 16)
%pause(5);
figure(1);hold on
contour(x,y,Utot);
%grid minor 
if (showAPF==1)
    figure(2);clc;
    surf(x,y,Utot);
    title('APF Resultante', 'Interpreter', 'latex', 'Fontsize', 14)
    figure(3)
    surf(x,y,Uatt1);
    title('APF de Atraccion', 'Interpreter', 'latex', 'Fontsize', 14)
    figure(4);clc;
    surf(x,y,Urep+Urepw); 
    title('APF de Repulsion', 'Interpreter', 'latex', 'Fontsize', 14)
end
